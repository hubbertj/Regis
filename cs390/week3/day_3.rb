...
Add an offspring rule to the knowledge base;                                                                            offspring( Y, X ) :- parent( X, Y ).
...
The Prolog variable Y in the expression  parent( tom, X ), parent( X, Y ) will yield Tom’s grandchildren.  Generalize this expression and create a rule in the knowledge base to identify grandparents.  
Run a number of queries against the knowledge base to verify its accuracy.
